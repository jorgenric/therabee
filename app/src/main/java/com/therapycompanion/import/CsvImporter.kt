    ...
    fun exportTemplate(context: Context): java.io.File? {
        return try {
            ...
            file.writeText(buildString {
                appendLine("# ================================================================")
                appendLine("# EXERCISE IMPORT TEMPLATE  —  lines beginning with # are ignored")
                appendLine("# ================================================================")
                appendLine("#")
                appendLine("# COLUMN FORMAT GUIDE")
                appendLine("# -------------------")
                appendLine("# name")
                appendLine("#   Plain text, up to 100 characters. Required.")
                appendLine("#   Example: Hip Flexor Stretch")
                appendLine("#")
                appendLine("# body_system")
                appendLine("#   Any label you like. No fixed list. Required.")
                appendLine("#   Example: Lower Extremity  |  Respiratory  |  Core")
                appendLine("#")
                appendLine("# instructions")
                appendLine("#   Plain text. Required.")
                appendLine("#   If your text contains commas, wrap the whole cell in double-quotes.")
                appendLine("#   Example: \"Breathe in for 4 counts, hold for 2, exhale for 6.\"")
                appendLine("#")
                appendLine("# duration")
                appendLine("#   WHOLE NUMBER OF MINUTES — must be a plain number, nothing else.")
                appendLine("#   Decimals are accepted and rounded up to the nearest minute.")
                appendLine("#   VALID:   1   2   5   10   1.5")
                appendLine("#   INVALID: \"5-10 sec hold\"   \"30 seconds\"   \"1-2 min\"   \"5 min\"")
                appendLine("#")
                appendLine("# frequency")
                appendLine("#   Controls how often the exercise is offered. Must be one of:")
                appendLine("#     Daily        — every day")
                appendLine("#     3xWeek       — three times per week")
                appendLine("#     2xWeek       — twice per week")
                appendLine("#     Weekly       — once per week")
                appendLine("#     AsTolerated  — no frequency limit, always eligible")
                appendLine("#   INVALID: \"1x daily\"   \"1-3x daily\"   \"daily x2\"   \"3x/day\"")
                appendLine("#")
                appendLine("# days")
                appendLine("#   Which days of the week the exercise is scheduled on.")
                appendLine("#     Daily / Everyday / Every day  — all 7 days")
                appendLine("#     Weekdays                      — Mon through Fri")
                appendLine("#     Weekends                      — Sat and Sun")
                appendLine("#     Mon,Wed,Fri                   — specific days (wrap in \"quotes\")")
                appendLine("#   Abbreviations: M  Tu  W  Th  F  Sa  Su")
                appendLine("#   Full names also accepted: Monday, Tuesday, Wednesday, ...")
                appendLine("#")
                appendLine("# priority")
                appendLine("#   How the scheduler ranks this exercise. Must be 1, 2, or 3.")
                appendLine("#     1 = essential    (also accepted: high)")
                appendLine("#     2 = important    (also accepted: medium)")
                appendLine("#     3 = supplemental (also accepted: low)")
                appendLine("#")
                appendLine("# notes")
                appendLine("#   Optional therapist note shown during the session. Leave blank to omit.")
                appendLine("#")
                appendLine("# active")
                appendLine("#   true or false. Defaults to true if the column is omitted.")
                appendLine("#")
                appendLine("# ================================================================")
                ...
                appendLine("Hip Flexor Stretch,Lower Extremity,Hold the stretch for 30 seconds each side.,1,3xWeek,\"Mon,Wed,Fri\",2,,true")
                appendLine("Quad Sets,Lower Extremity,Tighten quad and hold for 10 seconds.,2,2xWeek,Weekdays,2,Only if pain below 5/10,true")
                appendLine("Shoulder Rolls,Upper Extremity,Roll shoulders forward 10 times then backward 10 times.,2,Daily,Daily,3,,true")
                appendLine("Standing Balance,Lower Extremity,Stand on one foot for 30 seconds each side. Use wall for support if needed.,2,3xWeek,\"Tue,Thu,Sat\",1,,true")
            })
            file
